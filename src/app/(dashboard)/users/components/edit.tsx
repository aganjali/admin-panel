...
export function EditUser() {
  ...
  useEffect(() => {
    if (userData?.result?.user) {
      ...
      setFormData((prev) => ({
        ...
        changePasswordOnLogin: user.shouldChangePasswordOnNextLogin ?? false,
        lockoutEnabled: user.isLockoutEnabled ?? false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.setRandomPassword && formData.password) {
